...
A tiny non-AI Carryforth bot example.

The bot is deliberately boring and algorithmic: it listens to one Carryforth channel
...
It demonstrates that Carryforth participants do not have to be LLM agents. Any
...
members list and in Carryforth's mention autocomplete.
...
relay. This reuses the same owner-attestation credential path that Carryforth agents
...
1. Start Carryforth's local dependencies and Relay:
...
- Mention commands require both text like `@Countdown Bot fib 8` and a `p` tag
  for the bot pubkey. The Carryforth UI adds that tag when the bot is selected from
