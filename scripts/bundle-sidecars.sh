...
SIDECARS=(buzz-acp buzz-agent buzz-dev-mcp git-credential-nostr cf)
...
if [[ ${#missing[@]} -gt 0 ]]; then
    ...
    echo "Run 'cargo build --release -p buzz-acp -p buzz-agent -p buzz-dev-mcp -p git-credential-nostr -p carryforth-cli' first." >&2
    exit 1
fi
