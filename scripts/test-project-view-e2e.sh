...
for container in buzz-postgres buzz-redis buzz-minio; do
  ...
  if [[ "${status}" != "healthy" ]]; then
    ...
    echo "Project View E2E: ${container} did not become healthy" >&2
    exit 1
  fi
done

database_name="buzz_pv_e2e_$$_${RANDOM}"
if [[ ! "${database_name}" =~ ^buzz_pv_e2e_[0-9_]+$ ]]; then
  echo "Refusing unsafe scratch database name: ${database_name}" >&2
  exit 1
fi

profile="${PROJECT_VIEW_E2E_PROFILE:-dev}"
if [[ "${profile}" == "dev" ]]; then
...
for _ in $(seq 1 60); do
  if ! kill -0 "${relay_pid}" 2>/dev/null; then
    cat "${relay_log}" >&2
    echo "Project View E2E: Relay exited before readiness" >&2
    exit 1
  fi
...
if [[ "${status_code:-}" != "200" ]]; then
  cat "${relay_log}" >&2
  echo "Project View E2E: Relay did not become ready" >&2
  exit 1
fi
...
if [[ -n "${PROJECT_VIEW_TEST_ARCHIVE:-}" ]]; then
  ...
  cargo test -p buzz-test-client --test e2e_project_view -- \
    --ignored \
    --nocapture \
    --test-threads=1
fi
