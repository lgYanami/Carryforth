...
migration_count=$(postgres_value \
  "SELECT count(*) FROM _sqlx_migrations WHERE version IN (57, 58, 59) AND success")
[[ "$migration_count" == "3" ]] || fail "semantic migrations 0057/0058/0059 are not all successful"
...
virtual_result_constraint_count=$(postgres_value \
  "SELECT count(*) FROM pg_constraint WHERE conrelid='events'::regclass AND convalidated AND conname IN ('events_kind_not_semantic_graph_query_result','events_kind_not_project_context_coordinate_search_result')")
[[ "$virtual_result_constraint_count" == "2" ]] \
