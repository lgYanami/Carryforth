...
canary cohort。第一方图形客户端只验收Desktop；Mobile与Web不在阶段6范围。
...
脚本会先调用Stage 5 canary，在同一个独立scratch数据库中构造exact schema-v3前置状态。需要：

- 已激活或可由脚本激活的Hermit工具链；
- Docker可用，且本地PostgreSQL、Redis与MinIO容器可启动；
...
```bash
PROJECT_VIEW_STAGE5_NO_BUILD=1 ./scripts/test-project-view-stage6-canary.sh
```
...
### 2.1 Stage 5 exact前置状态

脚本先完成一个真实local v2 → v3 Resource / Guide cutover和一个独立empty-state direct-v3初始化。后续Context
验收只在前者上继续，复用exact Resource、Guide、Role、active Assignment与新Runtime generation。
...
### 2.3 managed Agent fence与canonical Context

脚本启动真实`buzz-acp`和fixture ACP child，从child进程环境读取current Runtime fence，并以managed Agent身份：

1. 为当前Role加入一个Resource；
2. 停止ACP并启动下一Runtime generation；
3. 证明旧Runtime fence不能加入Document；
4. 用当前fence加入同一Document的Live与Pinned revision 1坐标；
5. 列出并核对exact canonical三项集合。

Context便利命令每次都读取verified v3 snapshot并提交完整replacement；不做自动rebase。脚本还在Assignment
有效时预签名一个mutation，在Assignment结束后以有效NIP-98重新提交，必须得到Assignment conflict。
...
2026-08-01T12:37:18Z，本地真实canary完整通过：

- evidence：`test-results/stage6-canary/20260801T123618Z-1542000`；
- `artifact-digests.sha256`自身SHA-256：
  `424fc05ce7e321a40b7bb7dd4d4101028bd96b01f94a7a8f963f74c65c2361f8`；
- scratch数据库和本次进程已删除；
- workspace与Tauri `incremental`目录计数为0；
- 未执行任何部署、真实外部动作或broad rollout。
