...
迁移 `0041_meeting_v2_direct_action_finalization.sql` 会删除未发布的旧 Plan/Step 表。
...
2. 发布 migration `0041` 和 direct Relay/DB/SDK/CLI，保持 direct Create 关闭；
...
`0041` 删除旧 Plan/Step 表，不能靠降级二进制安全回滚到 planned v1。若 direct v2 出现
