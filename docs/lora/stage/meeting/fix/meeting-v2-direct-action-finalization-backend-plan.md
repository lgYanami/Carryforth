...
不得改写已经存在的 `0045_meeting_v2_action_finalization.sql`。新增：

```text
migrations/0046_meeting_v2_direct_action_finalization.sql
```

保留 `0045` 文件只是维护 migration ledger 和已执行数据库的 checksum，不代表保留旧 runtime。
应用完 `0046` 后，最终 schema 和代码中都不再存在旧 Plan/Step 执行结构。
...
旧 Plan、Step 和 attempt 的关系型投影随 `0046` 删除，不迁移到新表。原因是：
...
数据库或让 `0046` 清理这些 obsolete projection。
...
2. 发布 `0046`、direct Relay/DB/SDK/CLI，保持 direct Create 关闭；
...
- 增加 `0046` 前向迁移，删除旧 step schema 并重建最小 run/receipt 表；
...
- `0046` 在发现 planned v1 active Session 时中止；
...
| `buzz-db` | `0046` 清理旧 schema、最小 run/receipt、状态投影和终态 transaction |
