...
Buzz semantic indexing requires PostgreSQL 17 with pgvector 0.8.5 installed in
...
The repository Compose files and the Helm quickstart PostgreSQL subchart use
the pinned image and execute `CREATE EXTENSION IF NOT EXISTS vector` only while
initializing a new database volume. Existing volumes do not rerun init scripts.
...
4. use a privileged database channel to run
   `CREATE EXTENSION IF NOT EXISTS vector` in the Buzz database;
5. run `buzz-admin semantic preflight` with the ordinary Buzz database role;
...
the exact writer database. Buzz's preflight is deliberately read-only: seeing
...
For the first rollout, keep all Relay pods on:
...
2. run `buzz-admin semantic preflight` with the Buzz runtime role;
