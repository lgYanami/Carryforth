...
impl BaseRoleBriefV3Fixture {
    fn validate(&self) -> Result<(), String> {
        ...
        let role_directory = self
            .role_directory
            .as_object()
            .ok_or_else(|| "RoleBriefV3 Role Directory must be an object".to_owned())?;
        if role_directory
            .get("total_active_roles")
            ...
            || role_directory
                .get("omitted_active_roles")
                .and_then(Value::as_u64)
                != Some(0)
        {
            return Err("base RoleBriefV3 Role Directory is inconsistent".to_owned());
        }
        if !matches!(
