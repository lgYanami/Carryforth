//! `buzz documents` — verified Project Document v1 reads and writes.
...
async fn require_identity(client: &BuzzClient) -> Result<DocumentIdentity, CliError> {
...
async fn cmd_list(client: &BuzzClient, format: &OutputFormat) -> Result<(), CliError> {
...
async fn cmd_patch(client: &BuzzClient, request: PatchRequest) -> Result<(), CliError> {
    let PatchRequest {
        document_id,
        expected_revision,
        patch_file,
