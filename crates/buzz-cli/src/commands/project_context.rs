//! `buzz project-context` — verified Edge discovery and explicit maintenance.
...
async fn require_identity(client: &BuzzClient) -> Result<ProjectViewIdentity, CliError> {
...
fn context_document_output(
    ...
    let fetch_command = format!("buzz documents get {document_id} --content-only");
    match metadata {
        Some(DocumentMetadata::Active {
            title,
            summary,
...
mod tests {
    ...
    fn query_client(base_url: String) -> BuzzClient {
        BuzzClient::new(base_url, Keys::generate(), None, None).expect("query test client")
    }
    ...
    fn compact_query_shapes_cannot_contain_document_body() {
        ...
        let output = ContextDocumentOutput {
            ...
            updated_at: None,
            updated_by: None,
            fetch_command: format!("buzz documents get {document_id} --content-only"),
            unavailable_reason: None,
        };
