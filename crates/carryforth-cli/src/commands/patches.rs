...
pub async fn cmd_get_patch(client: &CarryforthClient, event: &str) -> Result<(), CliError> {
...
/// status kind either way. Shared by `cf issues status`.
...
pub async fn dispatch(cmd: crate::PatchesCmd, client: &CarryforthClient) -> Result<(), CliError> {
