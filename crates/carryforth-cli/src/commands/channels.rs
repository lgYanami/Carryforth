...
use crate::client::{
    ...
    print_create_response, CarryforthClient,
};
    ...
    fn from_event(event: &serde_json::Value) -> Option<Self> {
        ...
        for tag in tags {
            ...
            match key {
                ...
                // NIP-29 emits both `private` and `public` (Carryforth adds the latter).
...
pub async fn cmd_get_channel(client: &CarryforthClient, channel_id: &str) -> Result<(), CliError> {
...
pub async fn cmd_get_canvas(client: &CarryforthClient, channel_id: &str) -> Result<(), CliError> {
    validate_uuid(channel_id)?;
    let filter = serde_json::json!({
        "kinds": [40100],
        "#h": [channel_id]
            ...
            many => {
                ...
                return Err(CliError::Usage(format!(
                    "persona '{slug}' has {} live instances for this owner ({}); \
                     ...
                     the duplicate in Carryforth Desktop before creating the channel",
                    many.len(),
                    candidates.join(", ")
                )));
            }
...
/// `cf channels create --template <name>`: load a desktop-local channel
...
pub async fn cmd_join_channel(client: &CarryforthClient, channel_id: &str) -> Result<(), CliError> {
    let channel_uuid = parse_uuid(channel_id)?;
...
pub async fn cmd_leave_channel(
    client: &CarryforthClient,
    channel_id: &str,
) -> Result<(), CliError> {
    let channel_uuid = parse_uuid(channel_id)?;
...
pub async fn cmd_set_add_policy(client: &CarryforthClient, policy: &str) -> Result<(), CliError> {
    match policy {
    ...
    // NOTE: This gate covers only the `cf channels set-add-policy` CLI path.
...
mod tests {
    ...
    fn name_matches_substring_case_insensitive() {
        assert!(name_matches("Carryforth-Chat-Composer", "composer", false));
        assert!(name_matches(
            "Carryforth-Chat-Composer",
            "carryforth",
            false
        ));
    ...
    fn name_matches_exact_case_insensitive() {
        assert!(name_matches("Carryforth", "carryforth", true));
        assert!(!name_matches("Carryforth-Chat", "carryforth", true));
    }
    ...
    fn make_test_client() -> CarryforthClient {
        ...
        CarryforthClient::new("ws://localhost:3000".to_string(), keys, None, None)
