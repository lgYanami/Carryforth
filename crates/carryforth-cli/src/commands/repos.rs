...
async fn current_repo(client: &CarryforthClient, repo_id: &str) -> Result<Event, CliError> {
    validate_repo_id(repo_id)?;
...
async fn cmd_protect_list(client: &CarryforthClient, repo_id: &str) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::ReposCmd, client: &CarryforthClient) -> Result<(), CliError> {
