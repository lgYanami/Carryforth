...
pub async fn cmd_get_issue(client: &CarryforthClient, event: &str) -> Result<(), CliError> {
...
pub async fn cmd_issue_status(
    ...
    // Mirrors `cf patches status`: default a `p` tag to the repo owner
...
pub async fn dispatch(cmd: crate::IssuesCmd, client: &CarryforthClient) -> Result<(), CliError> {
    use crate::IssuesCmd;
    match cmd {
        IssuesCmd::Create {
            repo_owner,
