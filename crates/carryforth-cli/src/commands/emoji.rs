...
async fn cmd_list(client: &CarryforthClient) -> Result<(), CliError> {
...
async fn fetch_own_emoji(client: &CarryforthClient) -> Result<Vec<CustomEmoji>, CliError> {
...
async fn cmd_set(client: &CarryforthClient, shortcode: &str, url: &str) -> Result<(), CliError> {
...
async fn cmd_rm(client: &CarryforthClient, shortcode: &str) -> Result<(), CliError> {
    let normalized = buzz_sdk::normalize_custom_emoji_shortcode(shortcode)
        .map_err(|e| CliError::Other(format!("invalid shortcode: {e}")))?;
    let mut emojis = fetch_own_emoji(client).await?;
...
pub async fn dispatch(cmd: crate::EmojiCmd, client: &CarryforthClient) -> Result<(), CliError> {
    use crate::EmojiCmd;
    match cmd {
