//! `cf project-view` — typed reads and optimistic-concurrency mutations.
...
async fn cmd_get(client: &CarryforthClient, format: &OutputFormat) -> Result<(), CliError> {
    let identity = require_v3_capability(client).await?;
    let snapshot = read_verified_v3_snapshot(client, identity).await?;
...
async fn cmd_init_v3(client: &CarryforthClient, command_path: &str) -> Result<(), CliError> {
...
async fn require_v3_capability(client: &CarryforthClient) -> Result<ProjectViewIdentity, CliError> {
...
async fn submit_mutation(client: &CarryforthClient, event: Event) -> Result<String, CliError> {
...
mod tests {
    ...
    async fn ordinary_project_view_rejects_v2_only_capability_before_query() {
        ...
        let client = CarryforthClient::new(url, Keys::generate(), None, None)
            .expect("Project View test client");
