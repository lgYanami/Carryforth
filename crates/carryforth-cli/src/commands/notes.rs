//! `cf notes` — NIP-23 long-form editable notes (kind:30023).
//!
...
pub async fn fetch_by_slug(client: &CarryforthClient, slug: &str) -> Result<Vec<Event>, CliError> {
    let filter = serde_json::json!({
        "kinds": [KIND_LONG_FORM],
        "#d": [slug],
        "limit": 50,
...
pub async fn cmd_rm(client: &CarryforthClient, slug: &str) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::NotesCmd, client: &CarryforthClient) -> Result<(), CliError> {
    use crate::NotesCmd;
    match cmd {
        NotesCmd::Set {
            name,
