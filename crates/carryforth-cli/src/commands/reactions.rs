...
pub async fn cmd_get_reactions(client: &CarryforthClient, event_id: &str) -> Result<(), CliError> {
    validate_hex64(event_id)?;
    let filter = serde_json::json!({
        "kinds": [7],
        "#e": [event_id]
...
pub async fn dispatch(cmd: crate::ReactionsCmd, client: &CarryforthClient) -> Result<(), CliError> {
