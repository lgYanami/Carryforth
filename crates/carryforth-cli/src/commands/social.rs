...
pub async fn cmd_get_event(client: &CarryforthClient, event_id: &str) -> Result<(), CliError> {
...
pub async fn cmd_get_contact_list(client: &CarryforthClient, pubkey: &str) -> Result<(), CliError> {
    validate_hex64(pubkey)?;
    let filter = serde_json::json!({
        "kinds": [3],
        "authors": [pubkey],
...
pub async fn dispatch(cmd: crate::SocialCmd, client: &CarryforthClient) -> Result<(), CliError> {
    use crate::SocialCmd;
    match cmd {
