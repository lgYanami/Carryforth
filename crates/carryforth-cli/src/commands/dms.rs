...
use crate::client::{extract_d_tag, normalize_write_response, CarryforthClient};
...
pub async fn cmd_list_dms(client: &CarryforthClient, limit: Option<u32>) -> Result<(), CliError> {
...
pub async fn cmd_open_dm(client: &CarryforthClient, pubkeys: &[String]) -> Result<(), CliError> {
...
pub async fn cmd_hide_dm(client: &CarryforthClient, channel_id: &str) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::DmsCmd, client: &CarryforthClient) -> Result<(), CliError> {
    use crate::DmsCmd;
    match cmd {
