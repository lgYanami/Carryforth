//! `cf moderation` — community moderation queue, enforcement, and audit.
//!
...
async fn cmd_unban(client: &CarryforthClient, pubkey: &str) -> Result<(), CliError> {
    validate_hex64(pubkey)?;
...
async fn cmd_untimeout(client: &CarryforthClient, pubkey: &str) -> Result<(), CliError> {
    validate_hex64(pubkey)?;
...
async fn cmd_restricted(client: &CarryforthClient) -> Result<(), CliError> {
    let resp = client.get_authed("/moderation/restricted").await?;
    println!("{resp}");
    Ok(())
}

async fn cmd_audit(client: &CarryforthClient, limit: i64) -> Result<(), CliError> {
    let resp = client
