//! Desktop-local channel template loading for `cf channels create --template`.
//!
...
//! a crate, since carryforth-cli and desktop-tauri are independent crates and the
...
// Persisted Desktop data keeps the existing bundle identifier during the CLI
// cutover; changing it would make existing templates appear to disappear.
...
fn load_templates(path: &Path) -> Result<Vec<ChannelTemplateRecord>, CliError> {
    if !path.exists() {
        return Err(CliError::NotFound(format!(
            "no channel templates store found at {} (create a template in Carryforth Desktop first, \
...
mod tests {
    ...
    fn find_template_matches_case_insensitive() {
        let f = write_store(
            r#"[{"id":"t1","name":"Carryforth Team","createdAt":"x","updatedAt":"x"}]"#,
        );
        let t = find_template(f.path(), "carryforth team").expect("found");
        assert_eq!(t.name, "Carryforth Team");
    ...
    fn find_template_not_found_lists_available_names() {
        ...
        assert!(msg.contains("Carryforth Team"));
    ...
    fn load_templates_parses_full_roster() {
        let f = write_store(
            r##"[{
                "id":"t1","name":"Carryforth Team","channel_type":"forum","visibility":"private",
