...
pub async fn dispatch(command: AgentsCmd, client: &CarryforthClient) -> Result<(), CliError> {
    match command {
        AgentsCmd::DraftCreate {
            ...
            if let Some(obj) = output.as_object_mut() {
                ...
                obj.insert(
                    "message".into(),
                    "Draft sent to Carryforth Desktop for owner review. Nothing changes until the owner saves it."
                        .into(),
                );
            }
            println!("{output}");
...
/// Require `CARRYFORTH_AUTH_TAG` and parse the owner pubkey from it. Used only by
/// the `draft-create` and `draft-update` paths.
fn require_owner(client: &CarryforthClient) -> Result<PublicKey, CliError> {
    let hex = client
        .auth_tag_owner_hex()
        .ok_or_else(|| CliError::Auth("agent draft requests require CARRYFORTH_AUTH_TAG".into()))?;
...
pub(crate) async fn fetch_archived_snapshot(
    client: &CarryforthClient,
...
/// `cf agents archived`: read path over [`fetch_archived_snapshot`] for
/// direct invocation — a trust failure (state 3) is fatal here so a
/// verification command can never look like success.
async fn cmd_archived(client: &CarryforthClient) -> Result<(), CliError> {
