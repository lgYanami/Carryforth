...
impl CarryforthClient {
    ...
    ///
    /// - `CARRYFORTH_CONNECT_TIMEOUT_SECS` — TCP connect timeout (default 15 s)
    /// - `CARRYFORTH_TIMEOUT_SECS` — per-request total timeout (default 30 s)
    ///
            ...
            match send_result {
                ...
                Ok(resp) => {
                    ...
                    if !resp_was_success(status) {
                        ...
                        {
                            format!(
                                "{message} (CARRYFORTH_AUTH_TAG is set — it may be stale or revoked; try unsetting it)"
                            )
                        } else {
                            message
                        };
    ...
    async fn handle_response(&self, resp: reqwest::Response) -> Result<String, CliError> {
        if !resp.status().is_success() {
            ...
            let message = serde_json::from_str::<serde_json::Value>(&body)
                ...
                .unwrap_or(body);
            if status == 403 && std::env::var("CARRYFORTH_AUTH_TAG").is_ok() {
                let message = format!(
                    "{message} (CARRYFORTH_AUTH_TAG is set — it may be stale or revoked; try unsetting it)"
                );
                return Err(CliError::Relay {
...
mod retry_tests {
    ...
    fn env_duration_secs_parsing() {
        ...
        const KEY: &str = "CARRYFORTH_CLI_TEST_DURATION_SECS";
...
/// through `CarryforthClient` to verify behavioural properties — not implementation details.
#[cfg(test)]
mod retry_policy_tests {
    ...
    use super::{CarryforthClient, ProjectCommandDelivery};
    ...
    fn test_client(base_url: &str) -> CarryforthClient {
        let keys = Keys::generate();
        CarryforthClient::new(base_url.to_string(), keys, None, None).unwrap()
    }
...
mod tests {
    ...
    fn sign_event_unchecked_does_not_inject_ambient_auth_tag() {
        ...
        let client = CarryforthClient::new(
    ...
    fn with_auth_tag_omits_header_when_not_configured() {
        let keys = Keys::generate();
        let client = CarryforthClient::new("https://test.relay".into(), keys, None, None).unwrap();
