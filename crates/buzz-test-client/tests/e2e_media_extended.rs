...
fn tiny_jpeg() -> Vec<u8> {
    include_bytes!("../../buzz-media/tests/fixtures/android/sanitized/bitmap-srgb-sanitized.jpg")
        .to_vec()
}

fn tiny_png() -> Vec<u8> {
    include_bytes!("../../buzz-media/tests/fixtures/android/sanitized/bitmap-srgb-sanitized.png")
        .to_vec()
}
...
fn tiny_gif() -> Vec<u8> {
    ...
    gif.extend_from_slice(&1u16.to_le_bytes());
    gif.extend_from_slice(&[0x80, 0, 0]);
    gif.extend_from_slice(&[0xff; 3]);
    gif.extend_from_slice(&[0; 3]);
    gif.push(0x2c);
    ...
    gif.extend_from_slice(&1u16.to_le_bytes());
    gif.push(0);
    gif.extend_from_slice(&[2, 2, 0x4c, 1, 0]);
    gif.push(0x3b);
    gif
}

fn tiny_webp() -> Vec<u8> {
    include_bytes!("../../buzz-media/tests/fixtures/source/pixel-grid-2x2.webp").to_vec()
}
