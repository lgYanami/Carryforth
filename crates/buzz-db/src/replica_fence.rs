...
mod tests {
    ...
    fn fence_starts_closed_and_opens_on_advance() {
        ...
        // ReplicaFence stores PostgreSQL-compatible microsecond timestamps.
        // Normalize the boundary before asserting an exact round trip because
        // Utc::now() may expose sub-microsecond precision on some platforms.
        let ts = DateTime::from_timestamp_micros(Utc::now().timestamp_micros())
            .expect("current timestamp must be representable at microsecond precision");
        fence.advance(ts);
