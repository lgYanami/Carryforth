...
use buzz_core::kind::{
    ...
    KIND_MEETING_SPEECH_INTENT, KIND_MEETING_SUMMARY_COMMAND, KIND_MEMBER_ADDED_NOTIFICATION,
    ...
    KIND_PRODUCT_FEEDBACK, KIND_PROFILE, KIND_PROJECT_CONTEXT_COMMAND,
...
async fn ingest_event_inner(
        ...
        .is_some_and(|channel| channel.room_kind == "meeting")
    {
        ...
        return Err(IngestError::Rejected(format!(
            "invalid: kind {kind_u32} is not part of the canonical Meeting log"
        )));
    }
...
mod tests {
    ...
    fn project_document_command_uses_the_closed_global_write_seam() {
        ...
        assert!(is_global_only_kind(KIND_PROJECT_DOCUMENT_COMMAND));
    ...
    #[test]
    fn meeting_v1_commands_require_channels_write_scope() {
        let dummy = make_dummy_event();
        for kind in [
            KIND_MEETING_SPEECH_INTENT,
            KIND_MEETING_MODERATOR_COMMAND,
            ...
            assert_eq!(
                required_scope_for_kind(kind, &dummy).expect("known Meeting V1 command"),
                Scope::ChannelsWrite
            );
        }
    }
