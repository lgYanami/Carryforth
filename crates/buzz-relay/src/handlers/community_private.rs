...
/// Return whether an explicit Context filter asks for unsupported NIP-50
/// search. Context projections are metadata-only and are not an FTS surface.
#[must_use]
pub(crate) fn filter_is_project_context_search(filter: &Filter) -> bool {
...
/// Closed result of the Project Context structural read gate.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum ProjectContextReadDecision {
    /// Credential, principal, signer, and canonical projection state are ready.
    Allowed,
    /// The caller is not a current globally-authorized principal.
    Restricted,
    /// The caller is eligible, but structural readiness must fail closed.
    Unavailable(&'static str),
}

impl ProjectContextReadDecision {
    /// Whether stored Context events may be exposed to this caller.
    #[must_use]
    pub(crate) const fn allowed(self) -> bool {
        matches!(self, Self::Allowed)
    }
...
/// Resolve the complete Context point/query read gate before event lookup.
///
/// The capability flag intentionally does not participate: disabling stops
/// advertisement and attach while preserving member reads and detach recovery.
pub(crate) async fn project_context_read_decision(
    state: &AppState,
    community_id: buzz_core::CommunityId,
    pubkey: &[u8],
    scopes: &[Scope],
    channel_ids: Option<&[uuid::Uuid]>,
) -> Result<ProjectContextReadDecision, buzz_db::DbError> {
    if !credential_can_read_community_private(scopes, channel_ids) {
        return Ok(ProjectContextReadDecision::Restricted);
    }
    if !state
        .db
        .project_context_authorized_pubkey(community_id, pubkey)
        .await?
    {
        return Ok(ProjectContextReadDecision::Restricted);
    }
    ...
    if status.project_view_schema_version != 3 {
        return Ok(ProjectContextReadDecision::Unavailable("schema"));
    }
    if !state
        .db
    ...
    Ok(ProjectContextReadDecision::Allowed)
}
...
/// Add all Context protocol kinds to a query's deny set without discarding
/// exclusions installed by another private protocol gate.
pub(crate) fn exclude_project_context_kinds(
    query: &mut buzz_db::EventQuery,
    filter: &Filter,
    context_read_allowed: bool,
) {
    if context_read_allowed || !filter_can_match_project_context(filter) {
        return;
    }
    let excluded = query.excluded_kinds.get_or_insert_with(Vec::new);
...
#[must_use]
pub(crate) fn event_is_visible(
    kind: u32,
    document_read_allowed: bool,
    context_read_allowed: bool,
) -> bool {
    (!is_project_context_protocol_kind(kind) || context_read_allowed)
...
mod tests {
    ...
    fn wildcard_mixed_and_by_id_paths_fail_closed() {
        ...
        for kind in PROJECT_DOCUMENT_KINDS {
            assert!(!event_is_visible(kind as u32, false, false));
            assert!(event_is_visible(kind as u32, true, false));
        }
        assert!(event_is_visible(1, false, false));
    }
    ...
    #[test]
    fn project_context_visibility_follows_structural_readiness() {
        let wildcard = Filter::new();
        ...
        exclude_project_context_kinds(&mut query, &wildcard, false);
        let excluded = query.excluded_kinds.expect("excluded kinds");
        ...
        for kind in PROJECT_CONTEXT_KINDS {
            assert!(!event_is_visible(kind as u32, true, false));
            assert!(event_is_visible(kind as u32, true, true));
        }
    }
}
