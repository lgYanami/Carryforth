...
enum ProjectViewPageRequest {
    // Migration-only source read used by the explicit v2 -> v3 operator
    // cutover/Resource-review flow. Ordinary runtime capability discovery no
    // longer advertises v2, and v3 clients use the closed v3 scopes below.
    V2MigrationObjects {
        revision: u64,
    ...
    V2MigrationCurrentEntities {
        revision: u64,
        projection_generation: u64,
        after: Option<buzz_db::project_view::ProjectViewEntityCursor>,
        limit: u16,
    },
    /// Exact retained v2 metadata pointer for an explicit operator migration
    /// or recovery read. Ordinary Project View queries never enter this path.
    V2MigrationMeta,
    V3CurrentEntities {
        revision: u64,
        projection_generation: u64,
    ...
    V3RoleHistory {
        revision: u64,
        projection_generation: u64,
        filter: buzz_db::project_view::ProjectViewV3RoleHistoryFilter,
        after: Option<buzz_db::project_view::ProjectViewV3RoleHistoryCursor>,
        limit: u16,
    },
}

impl ProjectViewPageRequest {
    const fn is_v2_migration(&self) -> bool {
        matches!(
            self,
            Self::V2MigrationObjects { .. }
                | Self::V2MigrationCurrentEntities { .. }
                | Self::V2MigrationMeta
        )
    }
}
...
const COORDINATE_SEARCH_V2_QUERY_EXTENSION: &str =
    "carryforth_project_context_coordinate_search_v2";
const ONE_HOP_SEMANTIC_QUERY_EXTENSION: &str = "carryforth_project_context_one_hop_semantic_search";
const ONE_HOP_SEMANTIC_V2_QUERY_EXTENSION: &str =
    "carryforth_project_context_one_hop_semantic_search_v2";
const QUOTED_ONE_HOP_SEMANTIC_QUERY_EXTENSION: &[u8] =
    b"\"carryforth_project_context_one_hop_semantic_search\"";
const QUOTED_ONE_HOP_SEMANTIC_V2_QUERY_EXTENSION: &[u8] =
    b"\"carryforth_project_context_one_hop_semantic_search_v2\"";
...
#[derive(serde::Deserialize)]
#[serde(deny_unknown_fields)]
struct ProjectContextCoordinateSearchV2HttpFilter {
    kinds: Vec<u32>,
    authors: Vec<String>,
    #[serde(rename = "#p")]
    recipients: Vec<String>,
    limit: u64,
    #[serde(rename = "carryforth_project_context_coordinate_search_v2")]
    request: ProjectContextCoordinateSearchQuery,
}
...
#[derive(serde::Deserialize)]
#[serde(deny_unknown_fields)]
struct ProjectContextOneHopSemanticV2HttpFilter {
    kinds: Vec<u32>,
    authors: Vec<String>,
    #[serde(rename = "#p")]
    recipients: Vec<String>,
    limit: u64,
    #[serde(rename = "carryforth_project_context_one_hop_semantic_search_v2")]
    request: ProjectContextOneHopSemanticQuery,
}
...
pub(crate) fn parse_project_context_coordinate_search_http_query(
    ...
    if raw_filters[0]
        .get(COORDINATE_SEARCH_QUERY_EXTENSION)
        .and_then(Value::as_object)
        .is_some_and(|request| request.contains_key("coordinate_types"))
    {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            "invalid:coordinate_search:v2_filter_required",
        ));
    }
    ...
    if request.coordinate_types.is_some() {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            "invalid:coordinate_search:v2_filter_required",
        ));
...
/// Parse the exclusive filtered HTTP Coordinate-search v2 extension.
pub(crate) fn parse_project_context_coordinate_search_v2_http_query(
    exact_authenticated_body: &[u8],
    raw_filters: &[Value],
    relay_pubkey: &nostr::PublicKey,
    authenticated_caller: &nostr::PublicKey,
    ...
    let extension_count = raw_filters
        .iter()
        .filter(|filter| filter.get(COORDINATE_SEARCH_V2_QUERY_EXTENSION).is_some())
        .count();
    if extension_count == 0 {
        return Ok(None);
    }
    ...
    let mut filters: Vec<ProjectContextCoordinateSearchV2HttpFilter> =
        serde_json::from_slice(exact_authenticated_body).map_err(|_| {
            api_error(
                StatusCode::BAD_REQUEST,
                "invalid:coordinate_search:closed_filter",
    ...
    if request.coordinate_types.is_none() {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            "invalid:coordinate_search:type_filter_required",
        ));
    }
    Ok(Some(request))
}
...
pub(crate) fn parse_project_context_one_hop_semantic_http_query(
    ...
    if raw_filters[0]
        .get(ONE_HOP_SEMANTIC_QUERY_EXTENSION)
        .and_then(|request| request.get("scope"))
        .and_then(Value::as_object)
        .is_some_and(|scope| scope.contains_key("coordinate_types"))
    {
        return Err(one_hop_http_error(
            StatusCode::BAD_REQUEST,
            "invalid_input",
            "Filtered Edge Coordinate search requires the v2 surface",
            false,
            None,
        ));
    }
...
/// Parse the exclusive filtered one-hop semantic v2 extension.
pub(crate) fn parse_project_context_one_hop_semantic_v2_http_query(
    exact_authenticated_body: &[u8],
    raw_filters: &[Value],
    relay_pubkey: &nostr::PublicKey,
    authenticated_caller: &nostr::PublicKey,
    ...
    let extension_count = raw_filters
        .iter()
        .filter(|filter| filter.get(ONE_HOP_SEMANTIC_V2_QUERY_EXTENSION).is_some())
        .count();
    if extension_count == 0 {
        return Ok(None);
    }
    ...
    let mut filters: Vec<ProjectContextOneHopSemanticV2HttpFilter> =
        serde_json::from_slice(exact_authenticated_body).map_err(|_| {
            one_hop_http_error(
                StatusCode::BAD_REQUEST,
                "invalid_input",
    ...
    if !matches!(
        ...
        return Err(one_hop_http_error(
            StatusCode::BAD_REQUEST,
            "invalid_input",
            "Filtered one-hop v2 requires an Edge Coordinate type filter",
            false,
            None,
        ));
    }
...
async fn execute_project_context_coordinate_search_http_query(
    ...
    execute_coordinate_search_http_query(
        state,
        tenant,
...
async fn execute_project_context_coordinate_search_v2_http_query(
    state: &AppState,
    tenant: &TenantContext,
    exact_authenticated_body: &[u8],
    authenticated_caller: nostr::PublicKey,
...
async fn execute_coordinate_search_http_query(
    state: &AppState,
    tenant: &TenantContext,
    exact_authenticated_body: &[u8],
    authenticated_caller: nostr::PublicKey,
    ...
    let result = if filtered {
        crate::semantic_coordinate_search::execute_project_context_coordinate_search_v2(
            state,
            tenant.community(),
            authenticated_caller,
            nip98_auth_event_id,
        ...
        crate::semantic_coordinate_search::execute_project_context_coordinate_search(
            state,
            tenant.community(),
            authenticated_caller,
            nip98_auth_event_id,
...
async fn execute_project_context_one_hop_semantic_http_query(
    ...
    execute_one_hop_semantic_http_query(
        state,
        tenant,
        exact_authenticated_body,
        authenticated_caller,
...
async fn execute_project_context_one_hop_semantic_v2_http_query(
    state: &AppState,
    tenant: &TenantContext,
    exact_authenticated_body: &[u8],
    authenticated_caller: nostr::PublicKey,
...
async fn execute_one_hop_semantic_http_query(
    state: &AppState,
    tenant: &TenantContext,
    exact_authenticated_body: &[u8],
    authenticated_caller: nostr::PublicKey,
    ...
    let started = std::time::Instant::now();
    let result = if filtered {
        crate::semantic_one_hop_search::execute_project_context_one_hop_semantic_search_v2(
            state,
            tenant.community(),
            authenticated_caller,
            nip98_auth_event_id,
        ...
        crate::semantic_one_hop_search::execute_project_context_one_hop_semantic_search(
            state,
            tenant.community(),
            authenticated_caller,
            nip98_auth_event_id,
...
fn body_has_one_hop_semantic_extension(body: &[u8]) -> bool {
    body_has_exact_extension_key(body, QUOTED_ONE_HOP_SEMANTIC_QUERY_EXTENSION)
        || body_has_exact_extension_key(body, QUOTED_ONE_HOP_SEMANTIC_V2_QUERY_EXTENSION)
}

fn body_has_exact_extension_key(body: &[u8], quoted_key: &[u8]) -> bool {
    body.windows(quoted_key.len())
        .enumerate()
        .any(|(index, window)| {
            window == quoted_key
                && body[index + quoted_key.len()..]
                    .iter()
                    .copied()
...
fn parse_project_view_page_request(
    ...
    let scope = extension
        ...
        .ok_or_else(|| {
            api_error(
                StatusCode::BAD_REQUEST,
                "buzz_project_view scope is required",
            )
        })
        .and_then(|value| {
            value.as_str().ok_or_else(|| {
                api_error(
                    StatusCode::BAD_REQUEST,
                    "buzz_project_view scope must be a string",
                )
            })
        })?;
    ...
    let request = match scope {
        "v2_migration_meta" => {
            require_project_view_outer_filter(
                raw,
                relay_pubkey,
                buzz_core::kind::KIND_PROJECT_VIEW_META,
                buzz_sdk::project_view_v3::PROJECT_VIEW_V3_META_TAG,
            )?;
            const ALLOWED: &[&str] = &["scope"];
            reject_project_view_extension_fields(extension, ALLOWED)?;
            ProjectViewPageRequest::V2MigrationMeta
        }
        "v2_migration_objects" => {
            // Closed, read-only source page for the explicit operator
            // migration. Ordinary v2 projection filters remain rejected.
            require_project_view_outer_filter(
                raw,
                relay_pubkey,
                buzz_core::kind::KIND_PROJECT_VIEW_OBJECT,
                "buzz-project-view-v2-object",
            )?;
            const ALLOWED: &[&str] = &["scope", "revision", "projection_generation", "after"];
            reject_project_view_extension_fields(extension, ALLOWED)?;
            ProjectViewPageRequest::V2MigrationObjects {
                revision: project_view_safe_revision(extension, "revision")?,
                projection_generation: project_view_safe_revision(
                    extension,
                    "projection_generation",
                )?,
                after: extension
                    .get("after")
                    .map(parse_project_view_object_cursor)
                    .transpose()?,
                limit,
            }
        }
        "v2_migration_current_entities" => {
            // Keep this exact legacy scope solely as a read-only cutover input.
            // It cannot mutate canonical state and the DB reader pins schema 2.
            require_project_view_outer_filter(
                raw,
                relay_pubkey,
                buzz_core::kind::KIND_PROJECT_VIEW_OBJECT,
        ...
        PROJECT_VIEW_V3_ROLE_HISTORY_SCOPE => {
            ...
            reject_project_view_extension_fields(extension, ALLOWED)?;
            let entity_types = parse_project_view_history_entity_types(extension)?;
            let role_id = extension
                .get("role_id")
                .map(|value| parse_project_view_uuid(value, "role_id"))
                .transpose()?;
            let assignment_id = extension
                .get("assignment_id")
                .map(|value| parse_project_view_uuid(value, "assignment_id"))
                .transpose()?;
            let member_pubkey = extension
                .get("member_pubkey")
                .map(parse_project_view_member_pubkey)
                .transpose()?;
            ProjectViewPageRequest::V3RoleHistory {
                revision: project_view_safe_revision(extension, "revision")?,
                projection_generation: project_view_safe_revision(
                    extension,
                    "projection_generation",
                ...
                after: extension
                    .get("after")
                    .map(parse_project_view_history_cursor)
                    .transpose()?,
                limit,
            }
        }
        _ => {
            return Err(api_error(
                StatusCode::BAD_REQUEST,
                "buzz_project_view scope is unsupported",
            ));
        }
    };
    Ok(Some(request))
...
fn bridge_filter_has_forbidden_unscoped_project_view_projection(
    raw: &Value,
    filter: &nostr::Filter,
) -> bool {
    let explicit_v2_migration = raw
        .get("buzz_project_view")
        .and_then(|extension| extension.get("scope"))
        .and_then(Value::as_str)
        .is_some_and(|scope| {
            matches!(
                scope,
                "v2_migration_meta" | "v2_migration_objects" | "v2_migration_current_entities"
            )
        });
    !explicit_v2_migration
        && crate::handlers::project_view::filter_is_exclusively_project_view(filter)
        && crate::handlers::project_view::filter_has_unscoped_project_view_projection(filter)
}

fn require_project_view_outer_filter(
    raw: &serde_json::Map<String, Value>,
    relay_pubkey: &nostr::PublicKey,
    expected_kind: u32,
    expected_tag: &str,
) -> Result<(), (StatusCode, Json<Value>)> {
    if raw.get("kinds") != Some(&serde_json::json!([expected_kind]))
        || raw.get("#t") != Some(&serde_json::json!([expected_tag]))
        || raw.get("authors") != Some(&serde_json::json!([relay_pubkey.to_hex()]))
    {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
...
fn reject_project_view_extension_fields(
    extension: &serde_json::Map<String, Value>,
    allowed: &[&str],
) -> Result<(), (StatusCode, Json<Value>)> {
    if extension.keys().any(|key| !allowed.contains(&key.as_str())) {
...
fn parse_project_view_entity_cursor(
    value: &Value,
) -> Result<buzz_db::project_view::ProjectViewEntityCursor, (StatusCode, Json<Value>)> {
    ...
    if object.len() != 2 || !object.contains_key("entity_type") || !object.contains_key("entity_id")
    {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            "current-entity after requires exactly entity_type and entity_id",
        ));
    }
    let entity_type = parse_project_view_entity_type(
        object.get("entity_type").and_then(Value::as_str),
        &[
            buzz_project_view::v2::RoleContinuityEntity::Role,
            buzz_project_view::v2::RoleContinuityEntity::RoleAssignmentProposal,
            buzz_project_view::v2::RoleContinuityEntity::RoleAssignment,
            buzz_project_view::v2::RoleContinuityEntity::WorkCommitment,
            buzz_project_view::v2::RoleContinuityEntity::RoleCheckpoint,
            buzz_project_view::v2::RoleContinuityEntity::RoleHandoff,
        ],
        "after.entity_type",
    )?;
    let entity_id = object
        ...
        .ok_or_else(|| {
            api_error(
                StatusCode::BAD_REQUEST,
                "current-entity after is missing entity_id",
            )
        })
        .and_then(|value| parse_project_view_uuid(value, "after.entity_id"))?;
    Ok(buzz_db::project_view::ProjectViewEntityCursor {
        entity_type,
        entity_id,
    })
}

fn parse_project_view_object_cursor(
    value: &Value,
    ...
    if object.len() != 2 || !object.contains_key("object_type") || !object.contains_key("object_id")
    {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            "migration-object after requires exactly object_type and object_id",
        ));
    }
    ...
    let object_id = object
        ...
        .ok_or_else(|| {
            api_error(
                StatusCode::BAD_REQUEST,
                "migration-object after is missing object_id",
            )
        })
        .and_then(|value| parse_project_view_uuid(value, "after.object_id"))?;
...
fn parse_project_view_history_cursor(
    value: &Value,
) -> Result<buzz_db::project_view::ProjectViewV3RoleHistoryCursor, (StatusCode, Json<Value>)> {
    let object = value.as_object().ok_or_else(|| {
        api_error(
            StatusCode::BAD_REQUEST,
            "buzz_project_view history after must be an object",
        )
    })?;
    if object.len() != 3
        || !object.contains_key("project_revision")
        || !object.contains_key("entity_type")
        || !object.contains_key("entity_id")
    {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            "history after requires exactly project_revision, entity_type, and entity_id",
        ));
    }
    let project_revision = project_view_safe_revision(object, "project_revision")?;
    let entity_type = parse_project_view_entity_type(
        object.get("entity_type").and_then(Value::as_str),
    ...
    let entity_id = object
        ...
        .ok_or_else(|| {
            api_error(
                StatusCode::BAD_REQUEST,
                "history after is missing entity_id",
            )
        })
        .and_then(|value| parse_project_view_uuid(value, "after.entity_id"))?;
    Ok(buzz_db::project_view::ProjectViewV3RoleHistoryCursor {
        project_revision,
        entity_type,
        entity_id,
    })
...
fn parse_project_view_history_entity_types(
    extension: &serde_json::Map<String, Value>,
) -> Result<Vec<buzz_project_view::v2::RoleContinuityEntity>, (StatusCode, Json<Value>)> {
    let values = extension
        ...
        .ok_or_else(|| {
            api_error(
                StatusCode::BAD_REQUEST,
                "buzz_project_view entity_types must contain 1..=4 history types",
            )
        })?;
    let allowed = history_entity_types();
    let mut result = Vec::with_capacity(values.len());
    for value in values {
        let entity_type =
            parse_project_view_entity_type(value.as_str(), &allowed, "entity_types entry")?;
        if result.contains(&entity_type) {
            return Err(api_error(
                StatusCode::BAD_REQUEST,
                "buzz_project_view entity_types must not contain duplicates",
            ));
        }
...
const fn history_entity_types() -> [buzz_project_view::v2::RoleContinuityEntity; 4] {
    [
        buzz_project_view::v2::RoleContinuityEntity::RoleAssignmentProposal,
        buzz_project_view::v2::RoleContinuityEntity::RoleAssignment,
        buzz_project_view::v2::RoleContinuityEntity::RoleCheckpoint,
...
fn parse_project_view_entity_type(
    value: Option<&str>,
    allowed: &[buzz_project_view::v2::RoleContinuityEntity],
    field: &str,
) -> Result<buzz_project_view::v2::RoleContinuityEntity, (StatusCode, Json<Value>)> {
    let entity_type = match value {
        Some("role") => buzz_project_view::v2::RoleContinuityEntity::Role,
        Some("role_assignment_proposal") => {
            buzz_project_view::v2::RoleContinuityEntity::RoleAssignmentProposal
        }
        Some("role_assignment") => buzz_project_view::v2::RoleContinuityEntity::RoleAssignment,
        Some("work_commitment") => buzz_project_view::v2::RoleContinuityEntity::WorkCommitment,
        Some("role_checkpoint") => buzz_project_view::v2::RoleContinuityEntity::RoleCheckpoint,
        Some("role_handoff") => buzz_project_view::v2::RoleContinuityEntity::RoleHandoff,
        _ => {
            return Err(api_error(
                StatusCode::BAD_REQUEST,
                &format!("buzz_project_view {field} is invalid"),
            ));
        }
    };
    if !allowed.contains(&entity_type) {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            &format!("buzz_project_view {field} is not allowed for this scope"),
        ));
    }
    Ok(entity_type)
}

fn parse_project_view_uuid(
    value: &Value,
    field: &str,
) -> Result<uuid::Uuid, (StatusCode, Json<Value>)> {
    let raw = value.as_str().ok_or_else(|| {
        api_error(
            StatusCode::BAD_REQUEST,
            &format!("buzz_project_view {field} must be a canonical UUID"),
        )
    })?;
    let parsed = raw.parse::<uuid::Uuid>().map_err(|_| {
        api_error(
    ...
    if parsed.to_string() != raw {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            &format!("buzz_project_view {field} must be a canonical lowercase UUID"),
        ));
    }
    Ok(parsed)
}

fn parse_project_view_member_pubkey(
    value: &Value,
) -> Result<nostr::PublicKey, (StatusCode, Json<Value>)> {
    let raw = value.as_str().ok_or_else(|| {
        api_error(
            StatusCode::BAD_REQUEST,
            "buzz_project_view member_pubkey must be canonical lowercase hex",
        )
    })?;
    let parsed = nostr::PublicKey::from_hex(raw).map_err(|_| {
        api_error(
            StatusCode::BAD_REQUEST,
            "buzz_project_view member_pubkey must be canonical lowercase hex",
        )
...
fn exclude_private_protocols_if_unauthorized(
    ...
    if crate::handlers::project_view::filter_has_unscoped_project_view_projection(filter) {
...
async fn query_events_authed(
    ...
    if (body
        ...
        || body
            .windows(COORDINATE_SEARCH_V2_QUERY_EXTENSION.len())
            .any(|window| window == COORDINATE_SEARCH_V2_QUERY_EXTENSION.as_bytes()))
    ...
    let has_coordinate_search_v2_extension = raw_filters
        .iter()
        .any(|filter| filter.get(COORDINATE_SEARCH_V2_QUERY_EXTENSION).is_some());
    let has_one_hop_semantic_extension = raw_filters
        .iter()
        .any(|filter| filter.get(ONE_HOP_SEMANTIC_QUERY_EXTENSION).is_some());
    let has_one_hop_semantic_v2_extension = raw_filters
        .iter()
        .any(|filter| filter.get(ONE_HOP_SEMANTIC_V2_QUERY_EXTENSION).is_some());
    if usize::from(has_graph_semantic_extension)
        + usize::from(has_coordinate_search_extension)
        + usize::from(has_coordinate_search_v2_extension)
        + usize::from(has_one_hop_semantic_extension)
        + usize::from(has_one_hop_semantic_v2_extension)
        > 1
    {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
    ...
    if has_graph_semantic_extension
        ...
        || has_coordinate_search_v2_extension
        || has_one_hop_semantic_extension
        || has_one_hop_semantic_v2_extension
    {
    ...
    if has_one_hop_semantic_v2_extension {
        if let Some(query) = parse_project_context_one_hop_semantic_v2_http_query(
            body,
            &raw_filters,
            &state.relay_keypair.public_key(),
            &pubkey,
        )? {
            return execute_project_context_one_hop_semantic_v2_http_query(
                state,
                tenant,
                body,
                pubkey,
    ...
    } else if let Some(query) = parse_project_context_one_hop_semantic_http_query(
        body,
        &raw_filters,
        &state.relay_keypair.public_key(),
        &pubkey,
    ...
    // ordinary query path.
    if has_coordinate_search_v2_extension {
        if let Some(query) = parse_project_context_coordinate_search_v2_http_query(
            body,
            &raw_filters,
            &state.relay_keypair.public_key(),
            &pubkey,
        )? {
            return execute_project_context_coordinate_search_v2_http_query(
                state,
                tenant,
                body,
                pubkey,
    ...
    } else if let Some(query) = parse_project_context_coordinate_search_http_query(
        body,
        &raw_filters,
        &state.relay_keypair.public_key(),
        &pubkey,
    ...
    if raw_filters.iter().zip(&filters).any(|(raw, filter)| {
        bridge_filter_has_forbidden_unscoped_project_view_projection(raw, filter)
    }) {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
    ...
    let project_view_projection_signer =
        crate::handlers::project_view::configured_projection_signer(state);
        ...
        parse_project_view_page_request(&raw_filters, &state.relay_keypair.public_key())?
    {
        if page.is_v2_migration()
            && !state
                .db
                .project_view_v3_migration_reader_authorized_pubkey(tenant.community(), &pubkey)
                .await
                .map_err(|error| {
                    internal_error(&format!(
                        "Project View migration reader authorization lookup: {error}"
                    ))
                })?
        {
            return Err(api_error(
        ...
        let page_result = match page {
            ProjectViewPageRequest::V2MigrationMeta => state
                .db
                .project_view_v2_migration_meta(
                    tenant.community(),
                    &state.relay_keypair.public_key(),
                )
                .await
                .map(|event| buzz_db::project_view::ProjectViewSnapshotPage {
                    events: vec![event],
                }),
            ProjectViewPageRequest::V2MigrationObjects {
                revision,
            ...
            ProjectViewPageRequest::V3CurrentEntities {
                ...
                state
                    .db
                    .project_view_v3_current_entities_page(
                        tenant.community(),
                        &state.relay_keypair.public_key(),
                        revision,
                        projection_generation,
            ...
            ProjectViewPageRequest::V3RoleHistory {
                ...
                state
                    ...
                    .project_view_v3_role_history_page(
                        tenant.community(),
                        &state.relay_keypair.public_key(),
                        &buzz_db::project_view::ProjectViewV3RoleHistoryPageRequest {
        ...
        let page = page_result.map_err(|error| match error {
            ...
            buzz_db::project_view::ProjectViewReadError::InvalidCursor(_) => {
                api_error(StatusCode::BAD_REQUEST, "invalid:project_view:cursor")
            }
        ...
        match filter_events {
            Ok(stored_events) => {
                for se in stored_events {
                    ...
                    if !crate::handlers::project_view::projection_event_visible_for_filter(
                        filter,
                        &se.event,
...
async fn count_events_authed(
    ...
    for filter in &filters {
        ...
        // A v3 metadata filter also matches retained v2 metadata at the NIP-01
        // kind/tag layer. Require the shared strict per-event parser even if a
        // future database implementation makes `#t` COUNT filters pushable.
        let needs_v3_projection_filtering =
            crate::handlers::project_view::filter_requires_v3_projection_post_filter(filter);
...
mod tests {
    ...
    fn parse_coordinate_search_v2_filters(
        filters: &[Value],
        relay: &nostr::PublicKey,
        caller: &nostr::PublicKey,
    ) -> Result<Option<ProjectContextCoordinateSearchQuery>, (StatusCode, Json<Value>)> {
        let body = serde_json::to_vec(filters).expect("serialize Coordinate-search v2 body");
        parse_project_context_coordinate_search_v2_http_query(&body, filters, relay, caller)
    }
    ...
    fn parse_one_hop_semantic_v2_filters(
        filters: &[Value],
        relay: &nostr::PublicKey,
        caller: &nostr::PublicKey,
    ) -> Result<Option<ProjectContextOneHopSemanticQuery>, (StatusCode, Json<Value>)> {
        let body = serde_json::to_vec(filters).expect("serialize one-hop semantic v2 body");
        parse_project_context_one_hop_semantic_v2_http_query(&body, filters, relay, caller)
    }
    ...
    #[test]
    fn filtered_coordinate_surfaces_are_versioned_and_scope_closed() {
        let relay = Keys::generate().public_key();
        let caller = Keys::generate().public_key();
        let filter = buzz_semantic_query::ProjectContextCoordinateTypeFilter::new(vec![
            buzz_semantic_query::ProjectContextCoordinateType::Work,
        ])
        .expect("type filter");

        let mut coordinate_request = coordinate_search_fixture(uuid::Uuid::new_v4());
        coordinate_request.coordinate_types = Some(filter.clone());
        let v1_coordinate = coordinate_search_filter_fixture(&relay, &caller, &coordinate_request);
        assert!(parse_coordinate_search_filters(&[v1_coordinate], &relay, &caller).is_err());
        let v2_coordinate = serde_json::json!({
            "kinds": [KIND_PROJECT_CONTEXT_COORDINATE_SEARCH_RESULT],
            "authors": [relay.to_hex()],
            "#p": [caller.to_hex()],
            "limit": 1,
            COORDINATE_SEARCH_V2_QUERY_EXTENSION: coordinate_request,
        });
        assert!(
            parse_coordinate_search_v2_filters(&[v2_coordinate], &relay, &caller)
                .expect("v2 Coordinate filter")
                .is_some()
        );

        let edge_key = buzz_project_context::EdgeKey::from_hex(&"11".repeat(32)).expect("Edge key");
        let edge_request = ProjectContextOneHopSemanticQuery {
            request_id: uuid::Uuid::new_v4(),
            project_id: uuid::Uuid::new_v4(),
            query: "next frontend work".to_owned(),
            limit: 8,
            scope: buzz_semantic_query::OneHopSemanticScope::EdgeCoordinates {
                edge_key,
                coordinate_types: Some(filter),
            },
        };
        let v1_edge = one_hop_semantic_filter_fixture(&relay, &caller, &edge_request);
        assert!(parse_one_hop_semantic_filters(&[v1_edge], &relay, &caller).is_err());
        let v2_edge = serde_json::json!({
            "kinds": [KIND_PROJECT_CONTEXT_ONE_HOP_SEMANTIC_SEARCH_RESULT],
            "authors": [relay.to_hex()],
            "#p": [caller.to_hex()],
            "limit": 1,
            ONE_HOP_SEMANTIC_V2_QUERY_EXTENSION: edge_request,
        });
        assert!(
            parse_one_hop_semantic_v2_filters(&[v2_edge], &relay, &caller)
                .expect("v2 Edge Coordinate filter")
                .is_some()
        );

        let incident_request = one_hop_semantic_fixture(uuid::Uuid::new_v4());
        let v2_incident = serde_json::json!({
            "kinds": [KIND_PROJECT_CONTEXT_ONE_HOP_SEMANTIC_SEARCH_RESULT],
            "authors": [relay.to_hex()],
            "#p": [caller.to_hex()],
            "limit": 1,
            ONE_HOP_SEMANTIC_V2_QUERY_EXTENSION: incident_request,
        });
        assert!(parse_one_hop_semantic_v2_filters(&[v2_incident], &relay, &caller).is_err());
    }
    ...
    #[test]
    fn project_view_extension_separates_migration_v2_from_v3_runtime_scopes() {
        let relay = Keys::generate().public_key();
        let role_id = uuid::Uuid::new_v4();
        let legacy_active = serde_json::json!({
            "kinds": [buzz_core::kind::KIND_PROJECT_VIEW_OBJECT],
            "authors": [relay.to_hex()],
            "#t": ["buzz-project-view-active"],
            "limit": 100,
            "buzz_project_view": {
                "scope": "active_objects",
                "revision": 7,
                "projection_generation": 2,
            },
        });
        assert!(parse_project_view_page_request(&[legacy_active], &relay).is_err());
        ...
        assert!(parse_project_view_page_request(&[missing_scope], &relay).is_err());

        let migration_object_id = uuid::Uuid::new_v4();
        let migration_objects = serde_json::json!({
            "kinds": [buzz_core::kind::KIND_PROJECT_VIEW_OBJECT],
            "authors": [relay.to_hex()],
        ...
        assert!(matches!(
            parse_project_view_page_request(std::slice::from_ref(&migration_objects), &relay),
            Ok(Some(ProjectViewPageRequest::V2MigrationObjects {
                revision: 7,
                projection_generation: 2,
                after: Some(buzz_db::project_view::ProjectViewSnapshotCursor {
                    object_type: buzz_project_view::ProjectViewObjectType::Role,
                    object_id,
                }),
                limit: 500,
            })) if object_id == migration_object_id
        ));

        let mut malformed_object_cursor = migration_objects.clone();
        malformed_object_cursor["buzz_project_view"]["after"]["extra"] = serde_json::json!(true);
        assert!(parse_project_view_page_request(&[malformed_object_cursor], &relay).is_err());

        let mut unknown_object_type = migration_objects.clone();
        unknown_object_type["buzz_project_view"]["after"]["object_type"] =
            serde_json::json!("role_assignment");
        assert!(parse_project_view_page_request(&[unknown_object_type], &relay).is_err());

        let mut noncanonical_object_id = migration_objects.clone();
        noncanonical_object_id["buzz_project_view"]["after"]["object_id"] =
            serde_json::json!("AAAAAAAA-AAAA-4AAA-8AAA-AAAAAAAAAAAA");
        assert!(parse_project_view_page_request(&[noncanonical_object_id], &relay).is_err());

        let mut wrong_object_tag = migration_objects.clone();
        wrong_object_tag["#t"] = serde_json::json!(["buzz-project-view-v3-object"]);
        assert!(parse_project_view_page_request(&[wrong_object_tag], &relay).is_err());

        let mut unpinned_object_page = migration_objects.clone();
        unpinned_object_page["buzz_project_view"]["revision"] = serde_json::json!(0);
        assert!(parse_project_view_page_request(&[unpinned_object_page], &relay).is_err());
        ...
        assert!(matches!(
            parse_project_view_page_request(&[current], &relay),
            Ok(Some(ProjectViewPageRequest::V2MigrationCurrentEntities {
                revision: 7,
                projection_generation: 2,
                limit: 100,
                ..
        ...
        let migration_meta = serde_json::json!({
            "kinds": [buzz_core::kind::KIND_PROJECT_VIEW_META],
            "authors": [relay.to_hex()],
            "#t": [buzz_sdk::project_view_v3::PROJECT_VIEW_V3_META_TAG],
            "limit": 2,
            "buzz_project_view": {
                "scope": "v2_migration_meta",
            },
        });
        assert!(matches!(
            parse_project_view_page_request(&[migration_meta], &relay),
            Ok(Some(ProjectViewPageRequest::V2MigrationMeta))
        ));
        ...
        let history = serde_json::json!({
            ...
            "buzz_project_view": {
                ...
                "revision": 7,
                "projection_generation": 2,
                "entity_types": ["role_checkpoint", "role_handoff"],
                "role_id": role_id,
                "after": {
                    "project_revision": 5,
        ...
        assert!(matches!(
            parse_project_view_page_request(std::slice::from_ref(&history), &relay),
            Ok(Some(ProjectViewPageRequest::V3RoleHistory {
                revision: 7,
                projection_generation: 2,
                limit: 20,
                ..
        ...
        let mut legacy_scope = history.clone();
        legacy_scope["buzz_project_view"]["scope"] = serde_json::json!("role_history");
        assert!(parse_project_view_page_request(&[legacy_scope], &relay).is_err());

        let mut legacy_tag = history;
        legacy_tag["#t"] = serde_json::json!(["buzz-project-view-v2-entity"]);
        assert!(parse_project_view_page_request(&[legacy_tag], &relay).is_err());
    }
    ...
    fn project_view_page_request_marks_only_explicit_v2_migration_scopes() {
        ...
        assert!(v2_requests
            .iter()
            .all(ProjectViewPageRequest::is_v2_migration));
        ...
        assert!(v3_requests.iter().all(|request| !request.is_v2_migration()));
    }

    #[test]
    fn legacy_project_view_projection_filters_require_an_explicit_migration_scope() {
        let relay = Keys::generate().public_key();
        ...
        assert!(matches!(
            parse_project_view_page_request(std::slice::from_ref(&unscoped), &relay),
            Ok(None)
        ));
        let unscoped_filter: nostr::Filter =
            serde_json::from_value(unscoped.clone()).expect("parse unscoped legacy filter");
        assert!(
            bridge_filter_has_forbidden_unscoped_project_view_projection(
                &unscoped,
                &unscoped_filter,
            )
        );
        ...
        let scoped_filter: nostr::Filter =
            serde_json::from_value(scoped.clone()).expect("parse scoped migration filter");
        assert!(
            !bridge_filter_has_forbidden_unscoped_project_view_projection(&scoped, &scoped_filter,)
        );
        assert!(matches!(
            parse_project_view_page_request(&[scoped], &relay),
            Ok(Some(ProjectViewPageRequest::V2MigrationObjects {
                revision: 9,
                projection_generation: 3,
                after: None,
    ...
    #[test]
    fn project_view_history_scope_rejects_noncanonical_or_cross_scope_cursor_types() {
        let relay = Keys::generate().public_key();
        let filter = serde_json::json!({
            ...
            "buzz_project_view": {
                ...
                "revision": 7,
                "projection_generation": 2,
                "entity_types": ["role_checkpoint"],
                "after": {
                    "project_revision": 5,
                    "entity_type": "work_commitment",
                    "entity_id": uuid::Uuid::new_v4(),
                },
            },
        });
        assert!(parse_project_view_page_request(&[filter], &relay).is_err());
    }
    ...
    /// `cf moderation reports` / `audit` 401'd on a NIP-98 URL mismatch in
