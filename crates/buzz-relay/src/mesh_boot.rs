...
pub(crate) async fn run_demo_echo(
    ...
    loop {
        // `recv_validated` consumes the wire frame before awaiting its Redis
        // fence check, so recreating it after every non-draining tick can lose
        // an already-read frame. Keep one pinned receive future alive until it
        // completes; cancellation is deliberate only when the session drains.
        let frame = {
