...
/// Moderator-signed Meeting retrieval-summary metadata command.
...
mod tests {
    ...
    fn meeting_kinds_have_expected_registry_roles() {
        ...
        assert!(is_command_kind(KIND_MEETING_SUMMARY_COMMAND));
        assert!(!is_relay_only_kind(KIND_MEETING_SUMMARY_COMMAND));
        for kind in [
            KIND_MEETING_SPEECH_INTENT,
            KIND_MEETING_MODERATOR_COMMAND,
            KIND_MEETING_HUMAN_FLOOR_REQUEST,
