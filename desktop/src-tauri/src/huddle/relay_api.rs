...
pub(crate) async fn connect_audio_relay(
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            eprintln!("carryforth-desktop: audio relay pipeline exited: {e}");
        }
        ...
        loop {
            ...
            for chunk in samples.chunks(FRAME_SAMPLES) {
                ...
                let n = match encode_result {
                    Ok(n) => n,
                    Err(e) => {
                        eprintln!("carryforth-desktop: opus encode error: {e}");
                        continue;
                    }
                };
                if n > 0 {
...
pub(crate) async fn fetch_channel_members_with_roles(
    ...
    let events = query_relay(state, std::slice::from_ref(&filter))
        .await
        .map_err(|e| {
            eprintln!("carryforth-desktop: fetch channel members failed: {e}");
            e
        })?;
