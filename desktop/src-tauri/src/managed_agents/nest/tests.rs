...
fn ensure_nest_skill_dir_has_700_permissions() {
    ...
    // Symlinks (e.g. .goose/skills/carryforth-cli) are skipped by the chmod loop.
    for dir in [
        ".agents",
        ".agents/skills",
...
#[cfg(unix)]
#[test]
fn ensure_nest_retires_app_owned_legacy_skill_assets() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path().join(".buzz");
    ensure_nest_at(&root).unwrap();

    let legacy_canonical = root.join(LEGACY_CANONICAL_SKILL_DIR);
    fs::create_dir_all(&legacy_canonical).unwrap();
    fs::write(legacy_canonical.join("SKILL.md"), "legacy managed skill").unwrap();
    fs::write(legacy_canonical.join(".skill-version"), "4\n").unwrap();
    for dir in [".goose/skills", ".claude/skills", ".codex/skills"] {
        let legacy_link = root.join(dir).join("buzz-cli");
        std::os::unix::fs::symlink("../../.agents/skills/buzz-cli", legacy_link).unwrap();
    }

    ensure_nest_at(&root).unwrap();

    let new_skill = root.join(".agents/skills/carryforth-cli/SKILL.md");
    assert!(new_skill.exists(), "SKILL.md should exist at new path");
    assert!(fs::read_to_string(&new_skill)
        .unwrap()
        .contains("cf agents"));
    assert!(!legacy_canonical.exists());
    for dir in [".goose/skills", ".claude/skills", ".codex/skills"] {
        assert!(root.join(dir).join("buzz-cli").symlink_metadata().is_err());
    }
}
...
#[cfg(unix)]
#[test]
fn ensure_skill_symlinks_preserves_existing_real_provider_path() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path().join(".buzz");
    let real_dir = root.join(".claude/skills/carryforth-cli");
    ...
    assert!(
        ...
        "a user-owned provider skill directory must not be replaced"
    );
    assert_eq!(
...
fn ensure_skill_symlinks_skip_dangling_symlink() {
    ...
    let dangling = codex_skills.join("carryforth-cli");
...
#[test]
fn cli_link_name_prod_is_cf() {
    assert_eq!(cli_link_name(false), "cf");
}

#[test]
fn cli_link_name_dev_is_cf_dev() {
    assert_eq!(cli_link_name(true), "cf-dev");
}
...
fn ensure_cli_symlink_creates_symlink_prod() {
    ...
    fs::write(exe_parent.join("cf"), "binary").unwrap();

    let local_bin = tmp.path().join("local_bin");
    ensure_cli_symlink_at(&exe_parent, &local_bin, false).unwrap();
    let link = local_bin.join(cli_link_name(false));
    assert!(link.symlink_metadata().unwrap().file_type().is_symlink());
    assert_eq!(fs::read_link(&link).unwrap(), exe_parent.join("cf"));
}
...
fn ensure_cli_symlink_does_not_clobber_regular_file_prod() {
    ...
    let result = ensure_cli_symlink_at(&exe_parent, &local_bin, false);
    assert!(result.is_err());
...
fn ensure_cli_symlink_does_not_clobber_regular_file_dev() {
    ...
    fs::create_dir_all(&local_bin).unwrap();
    let link = local_bin.join(cli_link_name(true));
    fs::write(&link, "user-installed cf-dev binary").unwrap();

    let result = ensure_cli_symlink_at(&exe_parent, &local_bin, true);
    assert!(result.is_err());
    assert!(link.symlink_metadata().unwrap().file_type().is_file());
    assert_eq!(
        fs::read_to_string(&link).unwrap(),
        "user-installed cf-dev binary"
    );
}

#[cfg(unix)]
#[test]
fn ensure_cli_symlink_removes_only_owned_legacy_links() {
    let tmp = tempfile::tempdir().unwrap();
    let exe_parent = tmp.path().join("MacOS");
    fs::create_dir(&exe_parent).unwrap();
    fs::write(exe_parent.join("cf"), "binary").unwrap();
    ...
    let local_bin = tmp.path().join("local_bin");
    fs::create_dir_all(&local_bin).unwrap();
    let owned = local_bin.join("buzz");
    let unrelated = local_bin.join("buzz-dev");
    std::os::unix::fs::symlink(exe_parent.join("buzz"), &owned).unwrap();
    std::os::unix::fs::symlink("/opt/other/buzz", &unrelated).unwrap();

    ensure_cli_symlink_at(&exe_parent, &local_bin, false).unwrap();

    assert!(owned.symlink_metadata().is_err());
    assert_eq!(
        fs::read_link(&unrelated).unwrap(),
        std::path::PathBuf::from("/opt/other/buzz")
    );
    assert_eq!(
        fs::read_link(local_bin.join("cf")).unwrap(),
        exe_parent.join("cf")
    );
}
...
#[cfg(unix)]
#[test]
fn ensure_cli_symlink_preserves_unrelated_cf_symlink() {
    let tmp = tempfile::tempdir().unwrap();
    let exe_parent = tmp.path().join("MacOS");
    fs::create_dir(&exe_parent).unwrap();
    fs::write(exe_parent.join("cf"), "binary").unwrap();
    ...
    let local_bin = tmp.path().join("local_bin");
    fs::create_dir_all(&local_bin).unwrap();
    let link = local_bin.join("cf");
    std::os::unix::fs::symlink("/opt/cloud-foundry/cf", &link).unwrap();

    let result = ensure_cli_symlink_at(&exe_parent, &local_bin, false);
    assert!(result.is_err());
    assert_eq!(
        fs::read_link(&link).unwrap(),
        std::path::PathBuf::from("/opt/cloud-foundry/cf")
    );
}
