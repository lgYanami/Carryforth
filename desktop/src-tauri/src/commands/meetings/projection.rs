...
#[path = "projection/speech.rs"]
mod speech;
pub(super) use speech::parse_speech;
...
fn validate_floor_state(
    ...
    if state.baton_config.as_ref().is_some_and(|config| {
        ...
        return Err(MeetingReadError::Other(integrity_error(
            "Meeting State has an invalid Grant timing profile",
        )));
    }
