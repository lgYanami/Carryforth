...
test("community menu exposes only local actions", async ({ page }) => {
  await installMockBridge(page, {});
  await page.goto("/");
  ...
  await expect(
    page.getByRole("menuitem", { name: "Copy local Relay URL" }),
  ).toBeVisible();
  await expect(
    page.getByRole("menuitem", { name: "Add a community" }),
  ).toHaveCount(0);
  await expect(
    page.getByRole("menuitem", { name: "Community settings" }),
  ).toHaveCount(0);
});
