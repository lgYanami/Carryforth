...
/**
 * Cached relay origin (e.g. "https://relay.example.invalid"),
...
/**
 * The relay origin (e.g. `https://relay.example.invalid`) if it has
...
export function rewriteRelayUrl(url: string): string {
  ...
  // was typed with uppercase (for example, a legacy hosted Relay URL).
  if (cachedRelayOrigin) {
