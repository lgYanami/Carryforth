/**
 * Remark plugin that detects bare `carryforth://message?…` URLs in text nodes
 * and replaces each with a custom `message-link` HAST element. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `carryforth://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](carryforth://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /carryforth:\/\/message\?[^\s<>"')\]]+/g;
