...
test("carryforth://message deep-link routes in-app, not the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink(
    `carryforth://message?channel=${CHANNEL}&id=${MESSAGE}`,
    handlers,
  );
...
test("non-message carryforth:// URLs fall through to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink("carryforth://channel?foo=bar", handlers);
  assert.deepEqual(external, ["carryforth://channel?foo=bar"]);
  assert.equal(inApp.length, 0);
});

test("malformed carryforth://message URL falls back to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  // Matches isMessageLink (starts with carryforth://message?) but is missing the
  // required channel/id params, so parse fails and we don't navigate in-app.
  openPopoverLink("carryforth://message?nope=1", handlers);
  assert.deepEqual(external, ["carryforth://message?nope=1"]);
  assert.equal(inApp.length, 0);
});
