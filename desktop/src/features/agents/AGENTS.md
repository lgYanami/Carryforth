...
9. **The defaults modal is progressively disclosed.** An unset global config
   starts on the Built-in Agent-first deployment fallback and carries that visible
   harness into the next saved edit. The `progressive-defaults` disclosure
   preset therefore begins at Provider for Built-in Agent, then reveals Model,
