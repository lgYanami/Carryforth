# Using Third-Party Nostr Clients with Carryforth

Carryforth uses a Nostr Relay that speaks NIP-29 (Relay-based groups) natively. Third-party Nostr clients connect directly to the compatibility binary `buzz-relay` using NIP-29 and NIP-42 authentication. The old NIP-28 compatibility proxy has been removed.

## Community scope

Carryforth treats the Relay URL/domain as authoritative for the Community. The current
...
| **Edits (kind:40003)** | ⚠️ | Works on the wire but Carryforth-specific — no standard NIP-29 client renders these |
| **Rich content (kind:40002)** | ⚠️ | Works on the wire but Carryforth-specific — no standard NIP-29 client renders these |
...
| Kind | Tags | Content |
|------|------|---------|
| **39000** | `d=<uuid>`, `name`, `closed` (always); `about` (if description non-empty); `private` (if applicable); `hidden` (DM channels only) | Group metadata. **Note:** `closed` is always emitted per NIP-29 convention (Carryforth channels require explicit membership), but open channels are still readable/writable by non-members at runtime. The tag reflects the membership model, not access enforcement. |
